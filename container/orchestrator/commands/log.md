...
**Target path:** `/workspace/episodes/<YYYY-MM-DD>-<slug>.md` using today's UTC date. If the target already exists, append `-2`, `-3`, etc. until you find a free name.
...
- **Synthesis for the PI** — 3-6 sentences of narrative. The "if you read nothing else, read this" paragraph. Written for future-PI reading this in two weeks after forgetting the context.
- **Next session pickup** — 1-3 sentences. If a fresh claude session starts here and the PI says "continue this thread", what's the obvious first move? This is what makes `/clear` safe.
